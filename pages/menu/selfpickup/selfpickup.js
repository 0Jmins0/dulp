Page({
  ...
  loadCart() {
    const cart = wx.getStorageSync('cart_selfpickup') || [];
    this.setData({ cart });
  ...
  addToCart(e) {
    ...
    if (dish) {
      dish.count++;
      let cart = wx.getStorageSync('cart_selfpickup') || [];
      ...
      wx.setStorageSync('cart_selfpickup', cart);
  ...
  viewCart() {
    wx.navigateTo({ url: '/pages/cart/selfpickup_cart' });
  }
})